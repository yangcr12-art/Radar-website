...
- 仅用于比赛总结数据域的中英文映射与分组
- 支持导入双表头 Excel（第1行英文锚点、第2行中文锚点）
- 支持复合标题拆分（含 `射门 / 射正`、`夺回球权/低位/中位/高位` 特例）
- 支持手动新增、删除、下载 CSV
- 与球员项目对应表隔离
...
- 维护球队中英映射
- 维护球队颜色与形状
- 支持单队上传 logo 与批量导入 logo（按文件名匹配球队名）
- 从球员数据同步球队

## 3.8 比赛总结 / 球队数据

- 使用独立接口 `/api/match-data/*`，与球员数据域隔离
- 导入 Excel（必须包含 `Team` 列）
- 支持数据集切换、删除、搜索球队、选择主队/客队
- 指标列表按主客队共同可解析数值列生成（交集）
- 勾选指标一键导入比赛雷达图（原始值对比数据）
- 显示名称和分组优先使用“比赛项目对应表”

## 3.9 比赛总结 / 比赛雷达图

- 同一比赛数据集内支持主/客队双队雷达对比
- 雷达长度使用主客队原始值，不使用排名/百分位
- 支持主副标题、比分、主客队颜色编辑
- 支持主客队 logo 下拉调用（来源于球队对应表）
- 每项支持手动 `max`（`min` 固定 0），未设置时默认 `较大值 * 1.1`
- 支持导出 SVG/PNG
- 与原“雷达图生成器”草稿和状态隔离
